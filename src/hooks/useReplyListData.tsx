...
function useReplyListData({
  ...
  function loadData(): Promise<void> {
    ...
    return loadList(params).then(({ data }) => {
      if (data.length === 0) {
        setNodata(true)
      } else {
  ...
  function updateDataAfterReply(): void {
    ...
    next
      ...
      .then(() => {
        setTimeout(() => {
          const replyId = syncReplyList.current[0].objectId
          const ele = document.getElementById(replyId).getElementsByClassName(messageBodyStyle['bbs-msg-body'])[0]
          if (!ele) return
          highLightEle(ele)
        }, 100)
      })
