...
function MessageBody(props) {
  const { small, details, curNest, maxNest, loadList, updateCommentAsync } = props

  const editMessageRef = useRef(null)
  const { startReply, updateReplyDetails, updateComment } = useContext(ReplyUpdateContext)
  const { edit, editMessage, setEditMessage, saveEdit, showEdit, closeEdit } = useReplyEdit({
    details,
    editMessageRef,
    updateComment,
    updateCommentAsync
  ...
  const isOwnerComment = useMemo(() => loggedUser && loggedUser.id != null && loggedUser.id === details.user_id, [
    loggedUser,
    details.user_id
  ])
